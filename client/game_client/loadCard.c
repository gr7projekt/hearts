    ...
    for(int j = 0 ; j < 12 ; j++)
    {
        for(int i = 0 ; i < 12 - j ; i++)
        {
            if(player_client[0].game_hand[i].suit > player_client[0].game_hand[i+1].suit)
            {
                ...
                tmp_char_suit = player_client[0].game_hand[i].char_suit;
                tmp_value = player_client[0].game_hand[i].value;
                tmp_char_value = player_client[0].game_hand[i].char_value;
                ...
                player_client[0].game_hand[i].char_suit = player_client[0].game_hand[i+1].char_suit;
                player_client[0].game_hand[i].char_value = player_client[0].game_hand[i+1].char_value;


                player_client[0].game_hand[i+1].suit = tmp_suit;
                player_client[0].game_hand[i+1].value = tmp_value;
                player_client[0].game_hand[i+1].char_suit = tmp_char_suit;
                player_client[0].game_hand[i+1].char_value = tmp_char_value;
            }
        }
    }
}
