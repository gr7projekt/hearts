/**********
 *
 * Created by Grupp7 as part of course HI2016 @ KTH
 * 2016-06-03
 *
 *
 *
...
int main(int argc, char* argv[])
{
    ...
    if(port >= 40000 && port <= 50000) {
        ...
        int count=0;  //bara använd för att se antal loopar till krasch
        ...
        char *trick[4];
        char recievetable[13] = {"FF;EE;FF;FF;"};
        char tmptable[13];
        char myhand[40];
        char ip[20];
        ...
        getIP(ip); //läser in ip till servern från ip.txt
//        printf("ip: %s\n",ip);
        ...
        while (!quit) {
            ...
            recievehand(myhand, player_1, ip); //tar emot nya kort

            initSpritePosition(position_1, position_2, position_3, position_4, dropzone_pos, played_pos, coin_pos); // måste ladda om kortens position

            turn = 0; //återställa hur många kort spelade
            brokenHeart = false; //återställa brokenHeart
            for(int i=0;i<13;i++)
                picked[i] = false; //återställa lagda kort.
...
//                receiveontable(recievetable, player_1, ip);
                ...
                if(strlen(recievetable)==12){
                    ...
                    split(recievetable,';',trick);      //delar upp strängen till strängar i listan
                ...
                while (SDL_PollEvent(&e)) {
                        ...
                        if (myturn(trick, player_1))//strcmp(trick[player_1[0].id],"EE;") == 0) //ska endast g� att flytta korten om det �r min tur, +
                        {
                            if (SDL_GetMouseState(NULL, NULL) & SDL_BUTTON(SDL_BUTTON_LEFT)) {
                                ...
                                if (cardNr > -1) {
                                    ...
                                    if((cardNr>0 && !picked[cardNr-1]) && !picked[cardNr])   //undviker en bugg att två kort plockas upp men endast det högra fortsätter följa musen. Lägger tillbaka det undere kortet
                                    {
                                        position_1[cardNr-1].x = initial_pos[0].x-DISTANCE_X;
                                        position_1[cardNr-1].y = initial_pos[0].y;
                                    }
                                }
                            }
                        }
                    ...
                    if (click == false && cardNr > -1)        //cardNr> -1 m�ste finnas f�r att undvika en bugg att baksidan laddas hos spelaren och blockerar kort.
                    {
                        if (((position_1[cardNr].x + WIDTH / 2 > dropzone_pos[0].x) && (position_1[cardNr].x + WIDTH / 2 < dropzone_pos[0].x + dropzone_pos[0].w)) && ((position_1[cardNr].y + HEIGHT / 2 > dropzone_pos[0].y) && (position_1[cardNr].y + HEIGHT / 2 < dropzone_pos[0].y + dropzone_pos[0].h))) {
                            ...
                            if (checkCard(player_1, cardNr, &brokenHeart, picked, turn, trick, whoStarted)){

                                sendCard(player_1, cardNr, trick, recievetable, &port, tmptable, ip); //Här skickas det lagda kortet till servern.
