...
int main(int argc, char* argv[])
{
    ...
    if(port >= 40000 && port <= 50000) {
        ...
        position(player_1, player_2, player_3,
                 player_4);  //�ven absoluta platsen m�ste med som man f�r av server vid anslutning argv[2]
            ...
            while (turn < 13 && !quit) {
                ...
                while (SDL_PollEvent(&e)) {
                    if (e.type == SDL_QUIT) {
                        ...
                        mouse_x = e.button.x;
                        mouse_y = e.button.y;
                        if ((mouse_x < advertism_pos[0].w + advertism_pos[0].x && mouse_x > advertism_pos[0].x) &&
                            (mouse_y < advertism_pos[0].h + advertism_pos[0].y && mouse_y > advertism_pos[0].y)) {
                            openWeb();
                        }
                        if (clickButton(e,
                                        help.rect))//((mouse_x < help.rect.x+help.rect.w && mouse_x > help.rect.x) && (mouse_y < help.rect.y + help.rect.h && mouse_y > help.rect.y))
                        {
                            createWindow();
                        }
                    }
                        ...
                        if (myturn(trick,
                                   player_1))//strcmp(trick[player_1[0].id],"EE;") == 0) //ska endast g� att flytta korten om det �r min tur, +
                        {
                    ...
                    if (click == false && cardNr >
                                          -1)        //cardNr> -1 m�ste finnas f�r att undvika en bugg att baksidan laddas hos spelaren och blockerar kort.
                    {
                        if (((position_1[cardNr].x + WIDTH / 2 > dropzone_pos[0].x) &&
                             (position_1[cardNr].x + WIDTH / 2 < dropzone_pos[0].x + dropzone_pos[0].w)) &&
                            ((position_1[cardNr].y + HEIGHT / 2 > dropzone_pos[0].y) &&
                             (position_1[cardNr].y + HEIGHT / 2 < dropzone_pos[0].y + dropzone_pos[0].h))) {
                            ...
                            if (checkCard(player_1, cardNr, leadCard, brokenHeart, picked, turn, trick)) {
                                sendCard(player_1, cardNr, trick, recieved_trick, &port);
                ...
                {
                    if (player_1[0].game_hand[i].suit == 0)
                        SDL_RenderCopy(gRenderer, card[i], &gSpriteClipsClubs[player_1[0].game_hand[i].value],
                                       &position_1[i]);
                    else if (player_1[0].game_hand[i].suit == 1)
                        SDL_RenderCopy(gRenderer, card[i], &gSpriteClipsDiamonds[player_1[0].game_hand[i].value],
                                       &position_1[i]);
                    else if (player_1[0].game_hand[i].suit == 2)
                        SDL_RenderCopy(gRenderer, card[i], &gSpriteClipsHearts[player_1[0].game_hand[i].value],
                                       &position_1[i]);
                    else if (player_1[0].game_hand[i].suit == 3)
                        SDL_RenderCopy(gRenderer, card[i], &gSpriteClipsSpades[player_1[0].game_hand[i].value],
                                       &position_1[i]);
                    else
                        continue;
                }
