...
void sendCard(Player player_1[], int nr, char* trick[], char tmp_trick[], Uint16 port)
{
    char str[4];
...
//    printf("str[1]: %c\n",str[1]);
    ...
    tmp_trick[player_1[0].id*3] = str[0]; //*3 d� varje enskilld plats har XX; dvs 3 tecken.. pid*3 f�r startplatsen
    tmp_trick[player_1[0].id*3+1]=str[1];

    trick[player_1[0].id] = strdup(str);

    printf("send tmp_trick: %s\n",tmp_trick); //de här ska skickas

    strcpy((char *)p->data, tmp_trick);
